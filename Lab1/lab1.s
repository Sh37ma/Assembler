...
e_sprawdzPoprawnoscLiczby:
movq %rax, %rbp         #dlugosc tekstu
dec %rbp                #'\n'
movq $0, %rdi           #licznik
jmp e_sprawdzPoczatek

e_sprawdzPoczatek:         #sprawdz czy jest to kod ascii >= zero

        movb liczba_u8(, %rdi, 1), %bh
        cmp $POCZATEK, %bh
        jge e_sprawdzKoniec
        jmp e_koniecNiePoprawny


e_sprawdzKoniec:                #sprawdz czy jest to kod ascii <= siedem
        cmp $KONIEC, %bh
        jle e_poprawne
        jmp e_koniecNiePoprawny

e_poprawne:                     #znak poprawny, inkrementuj licznik i sprawdz czy wszystkie znaki
        inc %rdi
        cmp %rbp, %rdi
        jl e_sprawdzPoczatek
        jmp e_koniecSprawdzania

e_koniecNiePoprawny:            #jeden ze znaków jest nie poprawny, następuje przerwanie
